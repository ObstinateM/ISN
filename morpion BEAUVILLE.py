...
# D̩finition des variables pour check la win
ListeCoup = []
...
def clic(event):
    ...
    par ligne et colonne. Au bout de 3 affiche le message gagne.
    """
    ...
    for i in range(0,3):
        if(Liste[i][0]==Liste[i][1]==Liste[i][2]==1):
            Can.create_text(300, 175, text = 'Les ronds ont gagnes !', font = 'Arial 20 bold')
            Can.unbind('<Button-1>')
            gagne = False
    ...
    for i in range(0,3):
        if(Liste[i][0]==Liste[i][1]==Liste[i][2]==2):
            Can.create_text(300, 175, text = 'Les croix ont gagnees !', font = 'Arial 20 bold')
            Can.unbind('<Button-1>')
            gagne = False
...
def cancel():
    """Fonction qui annule le coup et le supprime de la Liste : ListeCoup
    """
...
def reset():
    """Fonction qui detruit et recreer le canvas pour reset la grille. Reset les variables.
    """
